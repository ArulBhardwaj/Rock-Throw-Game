class Rock {
  constructor(x, y, width, height, launcherAngle) {
    var options = {
      isStatic: true,
      density: 0.1
    };
    ...
    this.velocity = p5.Vector.fromAngle(launcherAngle);
    World.add(world, this.body);
  }

  shoot(launcherAngle) {
    launcherAngle += 90;
    this.velocity = p5.Vector.fromAngle(launcherAngle * (3.14 / 180));
  ...
  display() {
    ...
    if (this.body.velocity.y === 0) {
      tmpAngle = this.launcherAngle + 90;
    } else {
      tmpAngle =
